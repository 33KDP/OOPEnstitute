    ...
    echo '<div class="container " style="padding: 3%">';
        ...
        echo '<div>';
            while ($row = $search_query->fetch(PDO::FETCH_ASSOC)) {
                $query = "SELECT * FROM IndividualClass WHERE tutor_id = :tid AND student_id = :sid AND subject_id= :subId";

                $query = DBConn::getInstance()->getPDO()->prepare($query);
                $query->execute(array(':tid'=> $row['id'], ':sid'=> $curStudent->getstudentId(), ':subId' => $subjectID));
