...
$search_query = "SELECT Tutor.id, `User`.first_name, `User`.last_name, District.district, `User`.rating FROM Tutor 
                    JOIN District Join Tutor_Subject Join `User`
                    ON `User`.id = tutor.user_id and `User`.district_id = district.id and Tutor.id = Tutor_Subject.tutor_id
                    WHERE Tutor_Subject.Subject_id = '$subjectID' AND Tutor.availability_flag=0 ";
    ...
    echo '<div class="container " style="padding: 3%">';
        ...
        echo '<div>';
            while ($row = $search_query->fetch(PDO::FETCH_ASSOC)) {
                $query = "SELECT * FROM GroupClass WHERE tutor_id = :tid AND group_id = :gid";

                $query = DBConn::getInstance()->getPDO()->prepare($query);
                $query->execute(array(':tid'=> $row['id'], ':gid'=> $gid));
                ...
                if ($query->fetch(PDO::FETCH_ASSOC) == false) {
                    echo '<div class="card mx-auto rounded-3 border-0 shadow my-3">';
                        ...
                        $lastURL = array('subId' => $subjectID, 'district' => $district, 'rating' => $rating, 'gid'=>$gid);
                        $_SESSION ['lastURL'] = $lastURL;
                        echo '
                            <div class="card-body">
                                    ...
                                    <a href="../Student/viewTutor.php?tid=' . $id . '&sid=' . $subjectID . '&gid'.'" class="btn btn-secondary">View</a>
                                    <a href="../Class/submit.php?id=' . $id . '&sid=' . $subjectID .'&gid='.$gid.'&type=enroll" class="btn btn-primary">Enroll</a>
                            </div>
        ...
        echo '</div>';
            echo '<a href="joinClass.php?gid='.$_GET['gid'].'" class="btn btn-primary"> Back to Search</a><br/>';
    echo '</div>';
    ?>
