...
class GroupClass extends _Class
{
    ...
    function __construct($class_id)
    {
        ...
        $qry = $dbConn->getPDO()->prepare("SELECT GroupClass.tutor_id, `Group`.subject_id, `Group`.id FROM GroupClass JOIN `Group` ON GroupClass.group_id=`Group`.id WHERE GroupClass.id=:clsid");
